...
- The terminal tab lifecycle: connecting a non-tunnel connection adds a `TerminalTab` (with one or two `panes`) to `shellTabs` in `App.tsx` and switches to the terminals view. Closing a pane calls `ssh_disconnect`; closing a tab disconnects all its panes. All `Terminal` components stay mounted; only the active tab/pane receives `isVisible=true`.
